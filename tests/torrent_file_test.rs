...
fn test_debian_parsing() {
    ...
    assert_eq!(torrent.info.piece_hashes.len(), 3016);

    assert_eq!(torrent.announce, Some("http://bttracker.debian.org:6969/announce".to_string()));
    assert_eq!(
        torrent.comment,
...
fn test_wet_fly_fishing_parsing() {
    ...
    assert_eq!(
        torrent.announce_list,
        Some(vec!["http://bt3.t-ru.org/ann".to_string(), "http://retracker.local/announce".to_string()])
    );
    assert_eq!(
        torrent.info.name,
    ...
    assert_eq!(torrent.info.piece_hashes.len(), 880);

    assert_eq!(torrent.announce, Some("http://bt3.t-ru.org/ann".to_string()));
    assert_eq!(
        torrent.comment,
        Some("https://rutracker.org/forum/viewtopic.php?t=1786135".to_string())
    );
    assert_eq!(torrent.created_by, Some("uTorrent/1820".to_string()));
