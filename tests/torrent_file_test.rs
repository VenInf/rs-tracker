...
fn test_debian_parsing() {
    ...
    assert_eq!(torrent.info.name, "debian-13.4.0-amd64-netinst.iso");
    assert_eq!(torrent.info.piece_length, 262144);
    assert_eq!(torrent.info.piece_hashes.len(), 3016);

    assert_eq!(
        torrent.announce,
        Some("http://bttracker.debian.org:6969/announce".to_string())
    );
    assert_eq!(
        torrent.comment,
...
fn test_wet_fly_fishing_parsing() {
    ...
    assert_eq!(
        ...
        Some(vec![
            "http://bt3.t-ru.org/ann".to_string(),
            "http://retracker.local/announce".to_string()
        ])
    );
    assert_eq!(
        torrent.info.name,
    ...
    assert_eq!(
        torrent.announce,
        Some("http://bt3.t-ru.org/ann".to_string())
    );
    assert_eq!(
        torrent.comment,
